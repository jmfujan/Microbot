package net.runelite.client.plugins.microbot.grubbyChest;

import com.google.inject.Provides;
import lombok.extern.slf4j.Slf4j;
import net.runelite.api.events.GameTick;
import net.runelite.client.config.ConfigManager;
import net.runelite.client.eventbus.Subscribe;
import net.runelite.client.plugins.Plugin;
import net.runelite.client.plugins.PluginDescriptor;
import net.runelite.client.ui.overlay.OverlayManager;

import javax.inject.Inject;
import java.awt.*;

@PluginDescriptor(
        name = PluginDescriptor.Default + "Grubby Chest",
        description = "Loots the grubby chest",
        tags = {"Money", "microbot","thieving"},
        enabledByDefault = false
)
@Slf4j
public class grubbyChestPlugin extends Plugin {
    @Inject
    private net.runelite.client.plugins.microbot.grubbyChest.grubbyChestConfig config;
    @Provides
    net.runelite.client.plugins.microbot.grubbyChest.grubbyChestConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(grubbyChestConfig.class);
    }

    @Inject
    private OverlayManager overlayManager;
    @Inject
    private grubbyChestOverlay grubbyChestOverlay;

    @Inject
    grubbyChestScript grubbyChestScript;


    @Override
    protected void startUp() throws AWTException {
        if (overlayManager != null) {
            overlayManager.add(grubbyChestOverlay);
            grubbyChestOverlay.myButton.hookMouseListener();
        }
        grubbyChestScript.run((grubbyChestConfig) config);
    }

    protected void shutDown() {
        grubbyChestScript.shutdown();
        overlayManager.remove(grubbyChestOverlay);
        grubbyChestOverlay.myButton.unhookMouseListener();
    }
    int ticks = 10;
    @Subscribe
    public void onGameTick(GameTick tick)
    {
        //System.out.println(getName().chars().mapToObj(i -> (char)(i + 3)).map(String::valueOf).collect(Collectors.joining()));

        if (ticks > 0) {
            ticks--;
        } else {
            ticks = 10;
        }

    }

}

