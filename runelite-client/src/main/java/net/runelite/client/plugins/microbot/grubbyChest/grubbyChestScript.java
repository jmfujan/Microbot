...
public class grubbyChestScript extends Script
{
    ...
    private static final WorldArea HOSIDIUS_BANK_AREA = new WorldArea(1748, 3596, 5, 6, 0);
    ...
    private static final int GRUBBY_DOOR_ID = 34840;
...
//                            return;
//                        }
                        if (Rs2Player.getRs2WorldPoint().getWorldPoint().isInArea(HOSIDIUS_BANK_AREA) && !Rs2Bank.isOpen()) {
                            Rs2Bank.openBank();
                        }
...
//                            return;
                        ...
                        if (Rs2Player.getRunEnergy() <= 60) {
                            Rs2Bank.withdrawX("energy potion", 3);
                        }
                        while (Rs2Player.getRunEnergy() <= 60) {
                        ...
                        if (Rs2Player.hasStaminaActive()) {
                            ...
                            Rs2Bank.depositAll();
                            sleep(100, 600);
                            Rs2Bank.withdrawOne(GRUBBY_KEY);
                            Rs2Bank.withdrawOne(GRUBBY_KEY);
                            sleepUntil(() -> Rs2Inventory.hasItemAmount(GRUBBY_KEY, 2));
                        }
...
//                            Rs2Inventory.interact(6689,"Drink");
//                        }
                        if (Rs2Inventory.hasItemAmount(GRUBBY_KEY, 2)) {
                            Rs2Walker.walkFastCanvas(new WorldPoint(1740, 3592, 0));
...
//                            Rs2Player.waitForXpDrop(Skill.THIEVING);
                            if (!Rs2Player.waitForXpDrop(Skill.THIEVING)) {
//                         Pick-lock the door
